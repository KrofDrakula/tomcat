...
LeapMotionController.prototype = Object.create(GameController.prototype);

LeapMotionController.prototype.start = function(callback) {
    Leap.loop({ enableGestures: true }, this.handleFrame);
    callback();
};

LeapMotionController.prototype.handleFrame = function(frame) {
    if (this.previousFrame) {
        var translation = frame.translation(this.previousFrame);
        this.commandSet = {
            left  : translation[0] < -5,
            up    : translation[1] > 5,
            right : translation[0] > 5,
            down  : translation[1] < -5
        };
        this.emit('engage');
    }
...
LeapMotionController.prototype.getCommandSet = function() {
